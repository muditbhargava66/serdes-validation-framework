...
class TestHighBandwidthScope(unittest.TestCase):
    ...
    def test_configure_for_224g(self) -> None:
        ...
        for cmd in expected_commands:
            self.mock_controller.send_command.assert_any_call(
                self.scope_address,
                cmd
            )
        
        # Test with custom config
        custom_config = ScopeConfig(
            sampling_rate=240e9,
            bandwidth=100e9,
            timebase=10e-12,
            voltage_range=1.0
    ...
    def test_capture_eye_diagram(self) -> None:
        ...
        results = self.scope.capture_eye_diagram(
    ...
    def test_get_waveform_data(self) -> None:
        ...
        # Verify query
        self.mock_controller.query_instrument.assert_called_with(
            self.scope_address,
            ':WAVeform:DATA?'
        )
    ...
    def test_cleanup(self) -> None:
        ...
        self.mock_controller.disconnect_instrument.assert_called_with(
            self.scope_address
        )
