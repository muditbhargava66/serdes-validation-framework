...
class TestEthernet224GTestSequence(unittest.TestCase):
    ...
    def test_configure_scope_for_224g(self) -> None:
        ...
        config = {"sampling_rate": 256e9, "bandwidth": 120e9, "timebase": 5e-12, "voltage_range": 0.8}
        ...
        for cmd in expected_commands:
            self.mock_instrument_controller.send_command.assert_any_call(self.scope, cmd)
    ...
    def test_run_link_training_test(self) -> None:
        ...
        results = self.sequence.run_link_training_test(self.scope, self.pattern_gen, timeout_seconds=5.0)

        # Check result type
        self.assertIsInstance(results, TrainingResults)

        # Verify pattern generator configuration
        self.mock_instrument_controller.send_command.assert_any_call(self.pattern_gen, ":PATTern:TYPE PRBS31")

        # Verify scope configuration
        self.mock_instrument_controller.send_command.assert_any_call(self.scope, ":ACQuire:POINts 1000000")
        ...
        with self.assertRaises(AssertionError):
            self.sequence.run_link_training_test(self.scope, self.pattern_gen, timeout_seconds=-1.0)
    ...
    def test_measure_pam4_levels_basic(self) -> None:
        ...
        # Verify scope configuration
        self.mock_instrument_controller.send_command.assert_any_call(self.scope, ":ACQuire:SRATe 256000000000.0")
    ...
    def test_run_compliance_test_suite(self) -> None:
        ...
        results = self.sequence.run_compliance_test_suite(self.scope, self.pattern_gen)
    ...
    def test_capture_training_sequence(self) -> None:
        ...
        # Test normal case
        waveform = self.sequence._capture_training_sequence(self.scope, timeout_seconds=1.0)
    ...
    def test_compliance_criteria(self) -> None:
        ...
        mock_results = ComplianceResults(
            pam4_levels=PAM4Levels(
                level_means=np.array([-3.0, -1.0, 1.0, 3.0]), level_separations=np.array([2.0, 2.0, 2.0]), uniformity=0.1
            ),
            evm_results=EVMResults(rms_evm_percent=3.0, peak_evm_percent=6.0),
            eye_results=EyeResults(eye_heights=[0.3, 0.4, 0.3], eye_widths=[0.5, 0.5, 0.5]),
            jitter_results={"tj": 0.2e-12, "rj": 0.1e-12, "dj": 0.1e-12},
            test_status="PASS",
        )
