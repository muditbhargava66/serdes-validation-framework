...
from src.serdes_validation_framework.instrument_control.mode_switcher import (
    SignalMode as ModeSwitcherSignalMode,
)
from src.serdes_validation_framework.instrument_control.mode_switcher import (
...
from src.serdes_validation_framework.protocols.pcie.compliance import (
    ComplianceConfig,
    ComplianceTestSuite,
    ComplianceType,
)
from src.serdes_validation_framework.protocols.pcie.constants import (
    PCIE_SPECS,
    SignalMode,
    calculate_ui_parameters,
    validate_link_width,
)
from src.serdes_validation_framework.protocols.pcie.equalization import (
    create_lms_equalizer,
    create_rls_equalizer,
)
from src.serdes_validation_framework.protocols.pcie.link_training import (
    create_nrz_trainer,
    create_pam4_trainer,
)
...
class TestPCIeIntegration(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.TEST_SIGNAL_PARAMS = {"num_samples": 1000, "sample_rate": 100e9, "snr_db": 25.0}
    ...
    @classmethod
    def _generate_test_signal(cls, mode: SignalMode) -> dict[str, np.ndarray]:
        ...
        return {"time": time.astype(np.float64), "voltage": voltage.astype(np.float64)}
    ...
    def test_mode_switcher_integration(self) -> None:
        ...
        switcher = create_mode_switcher(default_mode=ModeSwitcherSignalMode.NRZ, sample_rate=50e9, bandwidth=25e9)
    ...
    def test_pcie_analyzer_integration(self) -> None:
        ...
        for mode in [SignalMode.NRZ, SignalMode.PAM4]:
            with self.subTest(mode=mode):
                ...
                config = PCIeConfig(
                    mode=mode, sample_rate=100e9, bandwidth=50e9, voltage_range=2.0, link_speed=32e9, lane_count=1
                )
    ...
    def test_compliance_testing_integration(self) -> None:
        ...
        results = test_suite.run_compliance_tests(self.nrz_signal["time"], self.nrz_signal["voltage"])
    ...
    def test_end_to_end_workflow(self) -> None:
        ...
        # 2. Signal analysis
        config = PCIeConfig(
            mode=SignalMode.PAM4, sample_rate=100e9, bandwidth=50e9, voltage_range=2.0, link_speed=32e9, lane_count=1
        )
        analyzer = PCIeAnalyzer(config)
        ...
        compliance_results = compliance_suite.run_compliance_tests(self.pam4_signal["time"], self.pam4_signal["voltage"])
    ...
    def test_error_handling(self) -> None:
        ...
        with self.assertRaises((AssertionError, ValueError)):
            config = PCIeConfig(
                mode=SignalMode.NRZ, sample_rate=100e9, bandwidth=50e9, voltage_range=2.0, link_speed=32e9, lane_count=1
            )
            analyzer = PCIeAnalyzer(config)
