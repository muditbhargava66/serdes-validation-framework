...
class TestPAM4Analyzer(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        try:
            ...
            samples_per_symbol = int(self.SIGNAL_PARAMS["SAMPLE_RATE"] / self.SIGNAL_PARAMS["SYMBOL_RATE"])
            ...
            noise = np.random.normal(0, self.SIGNAL_PARAMS["NOISE_AMPLITUDE"], self.SIGNAL_PARAMS["NUM_SAMPLES"])
            ...
            self.test_data = pd.DataFrame({"time": self.time, "voltage": self.noisy_signal}).astype(np.float64)
    ...
    def _generate_controlled_symbols(self, num_symbols: int) -> npt.NDArray[np.float64]:
        """
    ...
    def _create_filtered_signal(self, raw_signal: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
        """
    ...
    def _add_filtered_noise(self, clean_signal: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
    ...
    def test_calculate_evm(self) -> None:
        ...
        short_analyzer = PAM4Analyzer(
            {"voltage": short_signal, "time": np.arange(len(short_signal), dtype=np.float64)}
        )  # Cast time to float64
    ...
    def test_eye_diagram_analysis(self) -> None:
        ...
        self.assertTrue(all(h > self.TEST_THRESHOLDS["MIN_EYE_HEIGHT"] for h in results.eye_heights))
        self.assertTrue(all(w > self.TEST_THRESHOLDS["MIN_EYE_WIDTH"] for w in results.eye_widths))

        # Check worst case values
        self.assertGreater(results.worst_eye_height, self.TEST_THRESHOLDS["MIN_EYE_HEIGHT"])
        self.assertGreater(results.worst_eye_width, self.TEST_THRESHOLDS["MIN_EYE_WIDTH"])
    ...
    def test_normalize_signal(self) -> None:
        ...
        try:
            ...
            self.assertTrue(np.issubdtype(test_signal.dtype, np.floating), "Test signal must be floating-point type")
            self.assertEqual(len(test_signal), num_samples, "Test signal length mismatch")
            self.assertGreater(np.std(test_signal), 0, "Test signal must have non-zero variance")

            # Test normalization
            normalized = self.analyzer._normalize_signal(test_signal)

            # Test 1: Validate output type
            self.assertTrue(np.issubdtype(normalized.dtype, np.floating), "Normalized signal must be floating-point type")
            self.assertEqual(normalized.shape, test_signal.shape, "Signal shape must be preserved")

            # Test 2: Check centering (mean)
            mean = np.mean(normalized)
            self.assertAlmostEqual(mean, 0.0, delta=0.1, msg=f"Signal mean ({mean:.3e}) should be close to zero")
            ...
            self.assertGreater(signal_range, 2.0, msg=f"Dynamic range ({signal_range:.3f}) too small for PAM4")
            self.assertLess(signal_range, 8.0, msg=f"Dynamic range ({signal_range:.3f}) too large for PAM4")
            ...
            np.testing.assert_array_almost_equal(zero_norm, zero_signal, decimal=10, err_msg="Zero signal normalization failed")

            # Case 2: Constant signal
            const_signal = np.ones(100, dtype=np.float64)
            const_norm = self.analyzer._normalize_signal(const_signal)
            self.assertAlmostEqual(np.mean(const_norm), 0.0, delta=1e-10, msg="Constant signal not properly centered")

            # Test invalid inputs
            with self.assertRaises(AssertionError, msg="Should reject integer array"):
                self.analyzer._normalize_signal(np.array([1, 2, 3]))

            with self.assertRaises(AssertionError, msg="Should reject empty array"):
            ...
            self.assertFalse(np.any(np.isnan(tiny_norm)), "Normalization should handle tiny signals without NaN")
    ...
    def test_calculate_eye_widths(self) -> None:
        ...
        # Test normal operation
        widths = self.analyzer._calculate_eye_widths(time_normalized, normalized_signal)
        ...
        # Test error cases
        with self.assertRaises(AssertionError):
            self.analyzer._calculate_eye_widths(time_normalized, normalized_signal, threshold=-0.1)
    ...
    def test_signal_quality_metrics(self) -> None:
        ...
        freqs = np.fft.fftfreq(len(self.noisy_signal), 1 / self.SIGNAL_PARAMS["SAMPLE_RATE"])
        ...
        for edge in edges:
            if edge > 1 and edge < len(self.noisy_signal) - 2:
                ...
                rise_time = np.sum(np.diff(waveform) > 0) / self.SIGNAL_PARAMS["SAMPLE_RATE"]
