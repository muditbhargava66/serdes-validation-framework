...
class TestNRZAnalyzer(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.test_data = {"time": self.time_data, "voltage": self.nrz_signal}
    ...
    def _generate_test_signal(self) -> Tuple[npt.NDArray[np.float64], npt.NDArray[np.float64]]:
        ...
        voltage_signal = np.where(raw_signal == 1, signal_amplitude, -signal_amplitude).astype(np.float64)
    ...
    def test_validate_signal_arrays(self) -> None:
        ...
        with self.assertRaises(AssertionError):
            self.analyzer._validate_signal_arrays(self.time_data.astype(int), self.nrz_signal)

        with self.assertRaises(AssertionError):
            self.analyzer._validate_signal_arrays(self.time_data, self.nrz_signal.astype(int))

        # Test with mismatched lengths
        with self.assertRaises(AssertionError):
            self.analyzer._validate_signal_arrays(self.time_data[:-10], self.nrz_signal)

        # Test with empty arrays
        with self.assertRaises(AssertionError):
            self.analyzer._validate_signal_arrays(np.array([], dtype=np.float64), np.array([], dtype=np.float64))

        # Test with NaN values
        nan_array = self.nrz_signal.copy()
        nan_array[10] = np.nan
        with self.assertRaises(AssertionError):
            self.analyzer._validate_signal_arrays(self.time_data, nan_array)

        # Test with infinite values
        inf_array = self.nrz_signal.copy()
        inf_array[10] = np.inf
        with self.assertRaises(AssertionError):
            self.analyzer._validate_signal_arrays(self.time_data, inf_array)

        # Test with non-monotonic time
        non_monotonic = self.time_data.copy()
        non_monotonic[10] = non_monotonic[20]  # Create duplicate times
        with self.assertRaises(AssertionError):
            self.analyzer._validate_signal_arrays(non_monotonic, self.nrz_signal)
    ...
    def test_calculate_eye_width(self) -> None:
        ...
        eye_width = self.analyzer._calculate_eye_width(
            self.nrz_signal, self.time_data, ui_period=1.0 / self.SIGNAL_PARAMS["BIT_RATE"]
        )
        ...
        with self.assertRaises(ValueError):
            self.analyzer._calculate_eye_width(
                self.nrz_signal.astype(int), self.time_data, ui_period=1.0 / self.SIGNAL_PARAMS["BIT_RATE"]
            )

        # Test with invalid time data type
        with self.assertRaises(ValueError):
            self.analyzer._calculate_eye_width(
                self.nrz_signal, self.time_data.astype(int), ui_period=1.0 / self.SIGNAL_PARAMS["BIT_RATE"]
            )
        ...
        # Test with invalid UI period value
        with self.assertRaises(AssertionError):
            self.analyzer._calculate_eye_width(self.nrz_signal, self.time_data, ui_period=-1.0)
    ...
    def test_calculate_total_jitter(self) -> None:
        ...
        flat_analyzer = NRZAnalyzer(
            data=flat_data, sample_rate=self.SIGNAL_PARAMS["SAMPLE_RATE"], bit_rate=self.SIGNAL_PARAMS["BIT_RATE"]
        )
    ...
    def test_calculate_deterministic_jitter(self) -> None:
        ...
        corrupted_analyzer = NRZAnalyzer(
            data=self.test_data, sample_rate=self.SIGNAL_PARAMS["SAMPLE_RATE"], bit_rate=self.SIGNAL_PARAMS["BIT_RATE"]
        )
