...
class DualModeTestParameters:
    ...
    def __post_init__(self) -> None:
        ...
        assert self.transition_method in [
            "fast",
            "stable",
            "gradual",
...
class TransitionTestResult:
    ...
    def __post_init__(self) -> None:
        ...
        assert self.nrz_performance is None or isinstance(
            self.nrz_performance, TestResult
        ), "nrz_performance must be a TestResult instance or None"
        assert self.pam4_performance is None or isinstance(
            self.pam4_performance, TestResult
    ...
    def from_dict(cls, result_dict: Dict[str, Any]) -> "TransitionTestResult":
        ...
        required_keys = [
            ...
            "nrz_to_pam4_success_rate",
            "pam4_to_nrz_success_rate",
            "execution_time",
            "error_details",
        ]
...
class DualModeTestSequence:
    ...
    def __init__(self, pcie_gen: PCIeGen, parameters: Optional[DualModeTestParameters] = None) -> None:
        """
        ...
        self.nrz_sequence = PCIeTestSequence(
            pcie_gen=pcie_gen, signal_mode=SignalMode.NRZ, parameters=self.parameters.nrz_parameters
        )

        self.pam4_sequence = PCIeTestSequence(
            pcie_gen=pcie_gen, signal_mode=SignalMode.PAM4, parameters=self.parameters.pam4_parameters
        )
    ...
    def test_mode_transition(self, transition_mode: TransitionMode) -> TransitionTestResult:
        ...
        try:
            ...
            if transition_mode == TransitionMode.NRZ_TO_PAM4:
                ...
                transitions = [(SignalMode.NRZ, SignalMode.PAM4), (SignalMode.PAM4, SignalMode.NRZ)]
        ...
        try:
            ...
            for i in range(max_iterations):
                ...
                if eq_mode == EqualizationMode.NRZ_ONLY:
                    ...
                    nrz_taps = self._optimize_taps(nrz_taps, mode=SignalMode.NRZ)
                    pam4_taps = self._optimize_taps(pam4_taps, mode=SignalMode.PAM4)
                    compromise_taps = self._optimize_compromise_taps(nrz_taps, pam4_taps, nrz_weight, pam4_weight)
            ...
            # Create failure result
            result = EqualizationResult(converged=False, iterations=iterations, execution_time=time.time() - self._start_time)
    ...
    def _calculate_combined_performance(self, nrz_perf: float, pam4_perf: float, nrz_weight: float, pam4_weight: float) -> float:
        """
    ...
    def _optimize_compromise_taps(
        self, nrz_taps: List[float], pam4_taps: List[float], nrz_weight: float, pam4_weight: float
    ) -> List[float]:
        """
        ...
        if not nrz_taps or not pam4_taps:
            return self._initialize_compromise_taps(self._initialize_taps(SignalMode.NRZ), self._initialize_taps(SignalMode.PAM4))
