...
class PCIeTestSequenceConfig:
    ...
    def __post_init__(self) -> None:
        ...
        assert all(isinstance(lane, LaneConfig) for lane in self.lanes), \
            "All lanes must be LaneConfig instances"
        assert isinstance(self.test_phases, list), f"Test phases must be list, got {type(self.test_phases)}"
        assert all(isinstance(phase, PCIeTestPhase) for phase in self.test_phases), \
...
class SequenceResult:
    ...
    def __post_init__(self) -> None:
        ...
        assert isinstance(self.config, PCIeTestSequenceConfig), \
            f"Config must be PCIeTestSequenceConfig, got {type(self.config)}"
        assert isinstance(self.overall_status, PCIeTestResult), \
            f"Overall status must be PCIeTestResult, got {type(self.overall_status)}"
        assert isinstance(self.total_duration, float), \
            f"Total duration must be float, got {type(self.total_duration)}"
        assert isinstance(self.phase_results, list), \
            f"Phase results must be list, got {type(self.phase_results)}"
        assert isinstance(self.lane_results, dict), \
            f"Lane results must be dict, got {type(self.lane_results)}"
        
        assert self.total_duration >= 0, f"Total duration must be non-negative, got {self.total_duration}"
        assert all(isinstance(result, PhaseResult) for result in self.phase_results), \
...
class PCIeTestSequence:
    ...
    def __init__(self, config: PCIeTestSequenceConfig) -> None:
        ...
        """
        assert isinstance(config, PCIeTestSequenceConfig), \
            f"Config must be PCIeTestSequenceConfig, got {type(config)}"
    ...
    def run_complete_sequence(
        self,
        signal_data: Dict[int, Dict[str, npt.NDArray[np.float64]]]
    ) -> SequenceResult:
        """
    ...
    def _validate_signal_data(
        ...
        for lane_id, data in signal_data.items():
            ...
            assert 'time' in data and 'voltage' in data, \
                f"Lane {lane_id} must have 'time' and 'voltage' arrays"
            
            time_data = data['time']
            voltage_data = data['voltage']
            
            assert isinstance(time_data, np.ndarray), \
                f"Lane {lane_id} time data must be numpy array"
            assert isinstance(voltage_data, np.ndarray), \
                f"Lane {lane_id} voltage data must be numpy array"
            assert len(time_data) == len(voltage_data), \
                f"Lane {lane_id} time and voltage arrays must have same length"
    ...
    def _run_test_phase(
        ...
        try:
            ...
            return PhaseResult(
                phase=phase,
                status=PCIeTestResult.FAIL,
                duration=duration,
                metrics={},
                error_message=str(e)
            )
    
    def _run_initialization_phase(
        self,
        signal_data: Dict[int, Dict[str, npt.NDArray[np.float64]]]
    ) -> PhaseResult:
        ...
        try:
            ...
            return PhaseResult(
                phase=PCIeTestPhase.INITIALIZATION,
                status=status,
                duration=duration,
                metrics=metrics
            )
    ...
    def _run_link_training_phase(
        ...
        try:
            ...
            return PhaseResult(
                phase=PCIeTestPhase.LINK_TRAINING,
                status=status,
                duration=duration,
                metrics=metrics
            )
    ...
    def _run_compliance_phase(
        ...
        try:
            ...
            return PhaseResult(
                phase=PCIeTestPhase.COMPLIANCE,
                status=status,
                duration=duration,
                metrics=metrics
            )
    ...
    def _run_stress_test_phase(
        ...
        try:
            ...
            return PhaseResult(
                phase=PCIeTestPhase.STRESS_TEST,
                status=status,
                duration=duration,
                metrics=metrics
            )
    ...
    def _run_validation_phase(
        ...
        try:
            ...
            return PhaseResult(
                phase=PCIeTestPhase.VALIDATION,
                status=status,
                duration=duration,
                metrics=metrics
            )
...
def create_single_lane_nrz_test(
    lane_id: int = 0,
...
def create_multi_lane_pam4_test(
    num_lanes: int = 4,
    sample_rate: float = 200e9,
...
def create_comprehensive_pcie_test(
    mixed_modes: bool = True
) -> PCIeTestSequence:
    ...
    if mixed_modes:
        ...
        # All PAM4 lanes
        lanes = [
            LaneConfig(i, SignalMode.PAM4, 200e9, 100e9, 1.2)
            for i in range(4)
        ]
