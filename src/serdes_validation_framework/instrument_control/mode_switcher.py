...
class ModeConfig:
    ...
    def __post_init__(self) -> None:
        ...
        assert self.voltage_range > 0, \
            f"Voltage range must be positive, got {self.voltage_range}"
...
class SwitchResult:
    ...
    def __post_init__(self) -> None:
        ...
        assert isinstance(self.previous_mode, SignalMode), \
            f"Previous mode must be SignalMode, got {type(self.previous_mode)}"
        assert isinstance(self.new_mode, SignalMode), \
            f"New mode must be SignalMode, got {type(self.new_mode)}"
        assert isinstance(self.switch_time, float), \
            f"Switch time must be float, got {type(self.switch_time)}"
        
        # Value validation
        assert self.switch_time >= 0, \
            f"Switch time must be non-negative, got {self.switch_time}"
...
class ModeSwitcher:
    ...
    def __init__(
        ...
        assert isinstance(default_sample_rate, float), \
            f"Default sample rate must be float, got {type(default_sample_rate)}"
        assert isinstance(default_bandwidth, float), \
            f"Default bandwidth must be float, got {type(default_bandwidth)}"
        
        # Value validation
        assert default_sample_rate > 0, \
            f"Default sample rate must be positive, got {default_sample_rate}"
        assert default_bandwidth > 0, \
            f"Default bandwidth must be positive, got {default_bandwidth}"
    ...
    def get_mode_config(self, mode: SignalMode) -> ModeConfig:
        ...
        assert isinstance(mode, SignalMode), \
            f"Mode must be SignalMode, got {type(mode)}"
...
def create_mode_switcher(
    default_mode: SignalMode = SignalMode.NRZ,
    sample_rate: float = 50e9,
    ...
    assert isinstance(sample_rate, float), \
        f"Sample rate must be float, got {type(sample_rate)}"
    assert isinstance(bandwidth, float), \
        f"Bandwidth must be float, got {type(bandwidth)}"
