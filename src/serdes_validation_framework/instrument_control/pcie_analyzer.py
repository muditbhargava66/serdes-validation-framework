...
class PCIeConfig:
    ...
    def __post_init__(self) -> None:
        ...
        # Type validation
        assert isinstance(self.mode, SignalMode), "Mode must be SignalMode enum"
        assert isinstance(self.sample_rate, float), "Sample rate must be float"
        assert isinstance(self.bandwidth, float), "Bandwidth must be float"
        assert isinstance(self.voltage_range, float), "Voltage range must be float"
        assert isinstance(self.link_speed, float), "Link speed must be float"
        assert isinstance(self.lane_count, int), "Lane count must be integer"
...
class TrainingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        # Type validation
        assert isinstance(self.preset_index, int), "Preset index must be integer"
        assert isinstance(self.adaptation_mode, str), "Adaptation mode must be string"
        assert isinstance(self.target_snr, float), "Target SNR must be float"
        assert isinstance(self.max_iterations, int), "Max iterations must be integer"

        # Value validation
        assert 0 <= self.preset_index <= 10, "Preset index must be between 0 and 10"
        assert self.adaptation_mode in ["fast", "precise"], "Invalid adaptation mode"
...
class TrainingResults:
    ...
    def __post_init__(self) -> None:
        ...
        assert isinstance(self.iterations, int), "Iterations must be integer"
        assert isinstance(self.final_snr, float), "Final SNR must be float"
        assert all(isinstance(x, float) for x in self.tap_weights), "Tap weights must be floats"
        assert all(isinstance(x, float) for x in self.error_history), "Error history must be floats"
...
class PCIeAnalyzer:
    ...
    def __init__(self, config: PCIeConfig, mode_switcher: Optional[ModeSwitcher] = None) -> None:
        """
        ...
        """
        # Validate config
        assert isinstance(config, PCIeConfig), "Config must be PCIeConfig"

        self.config = config
        self.mode_switcher = mode_switcher or ModeSwitcher(
            default_mode=config.mode, default_sample_rate=config.sample_rate, default_bandwidth=config.bandwidth
        )
    ...
    def configure_mode(self, new_mode: SignalMode, mode_config: Optional[ModeConfig] = None) -> bool:
        """
        ...
        """
        # Validate input
        assert isinstance(new_mode, SignalMode), "New mode must be SignalMode enum"
    ...
    def analyze_signal(self, data: Dict[str, npt.NDArray[np.float64]]) -> Dict[str, float]:
        """
        ...
        try:
            ...
            # Store data
            self.current_data = {"time": data["time"].astype(np.float64), "voltage": data["voltage"].astype(np.float64)}
    ...
    def _validate_signal_arrays(self, time_data: npt.NDArray[np.float64], voltage_data: npt.NDArray[np.float64]) -> None:
        ...
        assert np.issubdtype(voltage_data.dtype, np.floating), "Voltage data must be floating-point"

        # Array validation
        assert len(time_data) == len(voltage_data), "Array length mismatch"
    ...
    def _analyze_nrz_signal(self) -> Dict[str, float]:
        ...
        try:
            ...
            return {"level_separation": level_separation, "snr_db": snr, "jitter_ps": jitter * 1e12}
    ...
    def _analyze_pam4_signal(self) -> Dict[str, float]:
        ...
        try:
            ...
            errors = voltage - ideal_levels[np.argmin(np.abs(voltage[:, np.newaxis] - ideal_levels), axis=1)]
            ...
            return {"min_level_separation": min_separation, "rms_evm_percent": rms_evm * 100, "snr_db": snr}

        except Exception as e:
            raise ValueError(f"PAM4 analysis failed: {e}")

    def _find_nrz_levels(self, histogram: npt.NDArray[np.float64], bin_edges: npt.NDArray[np.float64]) -> npt.NDArray[np.float64]:
        """
    ...
    def run_link_training(self, config: TrainingConfig) -> TrainingResults:
        """
    ...
    def _update_equalizer(self, tap_weights: List[float]) -> float:
        """
        ...
        try:
            # Validate input
            assert all(isinstance(x, float) for x in tap_weights), "Tap weights must be floats"
