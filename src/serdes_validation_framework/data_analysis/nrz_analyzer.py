...
class NRZLevels:
    ...
    def __post_init__(self) -> None:
        ...
        assert isinstance(self.level_separation, float), \
            "Level separation must be float"
        ...
        assert len(self.level_means) == 2, \
            f"Expected 2 NRZ levels, got {len(self.level_means)}"
            
        # Value validation
        assert self.level_separation > 0, \
            f"Level separation must be positive, got {self.level_separation}"
...
class EyeResults:
    ...
    def __post_init__(self) -> None:
        ...
        # Value validation
        assert self.eye_height >= 0, \
            f"Eye height must be non-negative, got {self.eye_height}"
        assert self.eye_width >= 0, \
            f"Eye width must be non-negative, got {self.eye_width}"
...
class JitterResults:
    ...
    def __post_init__(self) -> None:
        ...
        assert isinstance(self.deterministic_jitter, float), \
            "Deterministic jitter must be float"
...
class NRZAnalyzer:
    ...
    def _validate_signal_arrays(
        ...
        assert np.all(np.diff(time_data) > 0), \
            "Time data must be monotonically increasing"
    ...
    def _find_voltage_levels(
        ...
        assert np.issubdtype(histogram.dtype, np.floating), \
            "Histogram must be floating-point"
        assert np.issubdtype(bin_edges.dtype, np.floating), \
            "Bin edges must be floating-point"
        ...
        try:
            ...
            peaks, properties = find_peaks(
                ...
                prominence=np.max(histogram) * 0.2
            )
