...
class PAM4Analyzer(DataAnalyzer):
    ...
    def _normalize_signal(
        ...
        """
        Normalize signal with optimal level detection and scaling
        ...
        logger.info(f"Signal before normalization: {signal}")
        
        # 1. Remove DC offset using robust statistics
        dc_offset = np.median(signal)  # More robust than mean
        signal_centered = signal - dc_offset
        
        # 2. Find dominant signal levels using histogram
        hist, bins = np.histogram(signal_centered, bins=100)
        peaks, _ = scipy.signal.find_peaks(hist, distance=20, prominence=hist.max()*0.1)
        
        if len(peaks) >= 2:  # Found multiple levels
            # Use outermost peaks for scaling
            peak_voltages = bins[peaks]
            outer_range = np.max(peak_voltages) - np.min(peak_voltages)
            
            # Scale based on detected PAM4 range
            if outer_range > 0:
                target_range = 6.0  # -3 to +3
                scale = target_range / outer_range
                normalized = signal_centered * scale
                
                # Center around zero using the detected levels
                level_offset = np.mean([np.max(peak_voltages), np.min(peak_voltages)]) * scale
                normalized -= level_offset
                
                return normalized
                
        # Fallback to simple min-max scaling if level detection fails  
        min_val = np.min(signal_centered)
        max_val = np.max(signal_centered)
        norm_factor = 6.0 / (max_val - min_val)
        normalized = (signal_centered - min_val) * norm_factor - 3.0
        
        logger.info(f"Signal after normalization: {normalized}")
        logger.info(f"Mean absolute value of normalized signal: {np.mean(np.abs(normalized))}")
