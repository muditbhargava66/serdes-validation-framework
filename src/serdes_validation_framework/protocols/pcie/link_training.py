...
class EqualizerConfig:
    ...
    def __post_init__(self) -> None:
        ...
        """
        # Type validation
        assert isinstance(self.eq_type, EqualizerType), f"Equalizer type must be EqualizerType, got {type(self.eq_type)}"
        assert isinstance(self.num_taps, int), f"Number of taps must be int, got {type(self.num_taps)}"
        assert isinstance(self.tap_range, tuple), f"Tap range must be tuple, got {type(self.tap_range)}"
        assert len(self.tap_range) == 2, f"Tap range must have 2 elements, got {len(self.tap_range)}"
        assert all(isinstance(x, float) for x in self.tap_range), "Tap range values must be floats"
        assert isinstance(self.step_size, float), f"Step size must be float, got {type(self.step_size)}"

        # Value validation
        assert self.num_taps > 0, f"Number of taps must be positive, got {self.num_taps}"
        assert self.tap_range[0] < self.tap_range[1], f"Invalid tap range: {self.tap_range[0]} >= {self.tap_range[1]}"
        assert self.step_size > 0, f"Step size must be positive, got {self.step_size}"
...
class TrainingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        assert isinstance(self.max_iterations, int), f"Max iterations must be int, got {type(self.max_iterations)}"
        assert isinstance(
        ...
        assert self.max_iterations > 0, f"Max iterations must be positive, got {self.max_iterations}"
        assert (
            0 < self.convergence_threshold < 1
        ), f"Convergence threshold must be between 0 and 1, got {self.convergence_threshold}"

        # Equalizer validation
        assert all(isinstance(eq, EqualizerConfig) for eq in self.equalizers), "All equalizers must be EqualizerConfig instances"
...
class TrainingResult:
    ...
    def __post_init__(self) -> None:
        ...
        assert isinstance(self.final_ber, float), f"Final BER must be float, got {type(self.final_ber)}"
        assert isinstance(self.iterations, int), f"Iterations must be int, got {type(self.iterations)}"
        assert isinstance(self.phase, TrainingPhase), f"Phase must be TrainingPhase, got {type(self.phase)}"
        assert isinstance(self.equalizer_coeffs, dict), f"Equalizer coeffs must be dict, got {type(self.equalizer_coeffs)}"

        # Value validation
        assert self.final_ber >= 0, f"Final BER must be non-negative, got {self.final_ber}"
        ...
        for eq_type, coeffs in self.equalizer_coeffs.items():
            assert isinstance(eq_type, EqualizerType), f"Equalizer type must be EqualizerType, got {type(eq_type)}"
            assert isinstance(coeffs, list), f"Coefficients must be list, got {type(coeffs)}"
            assert all(isinstance(c, float) for c in coeffs), "All coefficients must be floats"

        # SNR history validation
        assert all(isinstance(snr, float) for snr in self.snr_history), "All SNR values must be floats"
...
class LinkTrainer:
    ...
    def __init__(self, config: TrainingConfig) -> None:
        ...
        """
        # Validate input
        assert isinstance(config, TrainingConfig), f"Config must be TrainingConfig, got {type(config)}"
    ...
    def run_training(self, signal_data: Dict[str, npt.NDArray[np.float64]]) -> TrainingResult:
        """
    ...
    def _validate_signal_data(self, signal_data: Dict[str, npt.NDArray[np.float64]]) -> None:
        """
    ...
    def _run_training_phase(self, signal_data: Dict[str, npt.NDArray[np.float64]], phase: TrainingPhase) -> bool:
        """
    ...
    def _run_preset_phase(self, signal_data: Dict[str, npt.NDArray[np.float64]]) -> bool:
        """
    ...
    def _run_optimization_phase(self, signal_data: Dict[str, npt.NDArray[np.float64]]) -> bool:
        """
    ...
    def _optimize_equalizer(self, signal_data: Dict[str, npt.NDArray[np.float64]], eq_config: EqualizerConfig) -> bool:
        """
        ...
        try:
            ...
            # Set bounds
            bounds = [(eq_config.tap_range[0], eq_config.tap_range[1]) for _ in range(len(current_coeffs))]

            # Run optimization
            result = minimize(objective, current_coeffs, method="L-BFGS-B", bounds=bounds, options={"maxiter": 100})
    ...
    def _run_fine_tuning_phase(self, signal_data: Dict[str, npt.NDArray[np.float64]]) -> bool:
        """
    ...
    def _run_verification_phase(self, signal_data: Dict[str, npt.NDArray[np.float64]]) -> bool:
        """
    ...
    def _calculate_ber(self, signal_data: Dict[str, npt.NDArray[np.float64]]) -> float:
        """
    ...
    def _calculate_snr(self, signal_data: Dict[str, npt.NDArray[np.float64]]) -> float:
        """
    ...
    def _find_signal_levels(self, voltage: npt.NDArray[np.float64], num_levels: int) -> npt.NDArray[np.float64]:
        """
    ...
    def _get_coeffs_dict(self) -> Dict[EqualizerType, List[float]]:
        ...
        """
        return {eq_type: coeffs.tolist() for eq_type, coeffs in self.equalizer_coeffs.items()}


# Factory functions
def create_nrz_trainer(target_ber: float = 1e-12, max_iterations: int = 1000) -> LinkTrainer:
    """
    ...
    """
    # Validate inputs
    assert isinstance(target_ber, float), f"Target BER must be float, got {type(target_ber)}"
    assert isinstance(max_iterations, int), f"Max iterations must be int, got {type(max_iterations)}"
    assert 0 < target_ber < 1, f"Target BER must be between 0 and 1, got {target_ber}"
    ...
    config = TrainingConfig(
        ...
        equalizers=[EqualizerConfig(eq_type=EqualizerType.TX_FFE, num_taps=5, tap_range=(-0.5, 0.5), step_size=0.01)],
    )

    return LinkTrainer(config)


def create_pam4_trainer(target_ber: float = 1e-12, max_iterations: int = 2000) -> LinkTrainer:
    """
    ...
    config = TrainingConfig(
        ...
        equalizers=[
            EqualizerConfig(eq_type=EqualizerType.TX_FFE, num_taps=7, tap_range=(-0.3, 0.3), step_size=0.005),
            EqualizerConfig(eq_type=EqualizerType.RX_CTLE, num_taps=3, tap_range=(-0.2, 0.2), step_size=0.01),
        ],
    )
