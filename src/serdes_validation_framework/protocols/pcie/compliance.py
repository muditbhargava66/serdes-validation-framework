...
class ComplianceConfig:
    ...
    def __post_init__(self) -> None:
        ...
        assert isinstance(self.record_length, float), f"Record length must be float, got {type(self.record_length)}"
        ...
        assert all(isinstance(t, ComplianceType) for t in self.test_types), "All test types must be ComplianceType enum values"
...
class ComplianceResult:
    ...
    def __post_init__(self) -> None:
        ...
        assert isinstance(self.measured_value, float), f"Measured value must be float, got {type(self.measured_value)}"
        assert isinstance(self.limit, ComplianceLimit), f"Limit must be ComplianceLimit, got {type(self.limit)}"
...
class ComplianceTestSuite:
    ...
    def __init__(self, config: ComplianceConfig) -> None:
        ...
        """
        # Validate input
        assert isinstance(config, ComplianceConfig), f"Config must be ComplianceConfig, got {type(config)}"

        self.config = config
        self.results: List[ComplianceResult] = []
        logger.info("Compliance test suite initialized")

    def validate_signal_data(self, time_data: npt.NDArray[np.float64], voltage_data: npt.NDArray[np.float64]) -> None:
        """
    ...
    def run_electrical_tests(self, voltage_data: npt.NDArray[np.float64]) -> Dict[str, ComplianceResult]:
        """
        ...
        try:
            ...
            results["voltage_swing"] = ComplianceResult(
                test_name="Voltage Swing", measured_value=v_pp, limit=ComplianceLimit(nominal=1.0, minimum=0.8, maximum=1.2)
            )
            ...
            results["common_mode"] = ComplianceResult(
                ...
                measured_value=v_cm,
                limit=ComplianceLimit(nominal=0.0, minimum=-0.2, maximum=0.2),
            )
    ...
    def generate_report(self) -> Dict[str, Union[bool, List[Dict[str, Union[str, float, bool]]]]]:
        ...
        return {"status": self.get_overall_status(), "results": formatted_results}


# Example usage
def create_test_suite(sample_rate: float = 20e9, record_length: float = 1e-6, voltage_range: float = 1.0) -> ComplianceTestSuite:
    """
