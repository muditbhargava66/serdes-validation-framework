...
class PAM4TrainingConfig:
    ...
    def __post_init__(self) -> None:
        ...
        assert isinstance(self.level_separation_mse_threshold, float), \
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> 'PAM4TrainingConfig':
        ...
        required_keys = [
            'voltage_levels', 'pre_cursor_taps', 'post_cursor_taps',
            'level_separation_mse_threshold', 'eye_height_threshold',
            'training_pattern', 'max_iterations', 'timeout_seconds'
        ]
...
class PAM4TrainingResults:
    ...
    def from_dict(cls, results_dict: Dict[str, Any]) -> 'PAM4TrainingResults':
        ...
        # Ensure all required keys are present
        required_keys = [
            'success', 'phase_reached', 'iterations', 'final_voltage_levels',
            'final_eye_heights', 'final_snr', 'level_mse', 'training_time',
            'voltage_adjustments_history', 'tap_adjustments_history'
        ]
        for key in required_keys:
...
class PAM4Trainer:
    ...
    def train(self) -> PAM4TrainingResults:
        ...
        try:
            ...
            success = self._detect_phase() and \
                     self._coarse_tuning_phase() and \
                     self._fine_tuning_phase() and \
                     self._level_adjustment_phase() and \
                     self._signal_integrity_phase()
    ...
    def _calculate_level_separation_mse(self) -> float:
        ...
        squared_errors = [
            (latest_levels[i] - ideal_levels[i]) ** 2
            for i in range(3)
        ]
