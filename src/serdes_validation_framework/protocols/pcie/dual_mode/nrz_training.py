...
class NRZTrainingConfig:
    ...
    def from_dict(cls, config_dict: Dict[str, Any]) -> 'NRZTrainingConfig':
        ...
        required_keys = [
            'voltage_swing', 'pre_emphasis', 'de_emphasis', 'eq_boost',
            'training_pattern', 'max_iterations', 'convergence_threshold',
            'timeout_seconds'
        ]
...
class NRZTrainingResults:
    ...
    def from_dict(cls, results_dict: Dict[str, Any]) -> 'NRZTrainingResults':
        ...
        # Ensure all required keys are present
        required_keys = [
            'success', 'phase_reached', 'iterations', 'final_eye_height',
            'final_eye_width', 'ber_estimate', 'training_time',
            'voltage_adjustments', 'emphasis_adjustments'
        ]
        for key in required_keys:
...
class NRZTrainer:
    ...
    def train(self) -> NRZTrainingResults:
        ...
        try:
            ...
            success = self._detect_phase() and \
                     self._polling_phase() and \
                     self._config_phase() and \
                     self._recovery_phase() and \
                     self._loopback_phase()
    ...
    def _check_convergence(self) -> bool:
        ...
        v_changes = [abs(self._voltage_history[-i] - self._voltage_history[-i-1]) 
                    for i in range(1, 3)]
        
        # Calculate recent emphasis changes
        e_changes = [abs(self._emphasis_history[-i] - self._emphasis_history[-i-1]) 
                    for i in range(1, 3)]
