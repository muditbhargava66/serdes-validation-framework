...
class ComplianceSpecification:
    ...
    def _initialize_limits(self) -> None:
        ...
        self.limits: Dict[str, ComplianceLimit] = {
            "level_separation": ComplianceLimit(nominal=0.5, minimum=0.4, maximum=0.6),
            "eye_height": ComplianceLimit(nominal=0.3, minimum=0.2, maximum=0.4),
            "eye_width": ComplianceLimit(nominal=0.5, minimum=0.4, maximum=0.6),
            "rms_evm": ComplianceLimit(nominal=3.5, minimum=0.0, maximum=5.0),
        }

    def check_pam4_levels(self, measured_levels: npt.NDArray[np.float64]) -> Tuple[bool, Dict[str, float]]:
        """
        ...
        assert np.issubdtype(measured_levels.dtype, np.floating), "Levels must be floating-point numbers"
        ...
        try:
            ...
            measurements = {"min_separation": min_separation, "uniformity": float(np.std(level_gaps) / np.mean(level_gaps))}
    ...
    def check_eye_diagram(self, eye_height: float, eye_width: float) -> Tuple[bool, Dict[str, float]]:
        """
    ...
    def check_evm(self, rms_evm: float, peak_evm: float) -> Tuple[bool, Dict[str, float]]:
        """
        ...
        try:
            ...
            measurements = {"rms_evm": rms_evm, "peak_evm": peak_evm, "margin": evm_limit.maximum - rms_evm}
    ...
    def get_test_config(self, test_type: str) -> ComplianceTestConfig:
        """
        ...
        try:
            configs = {
                "jitter": ComplianceTestConfig(
                    test_pattern=self.test_patterns["jitter"], sample_rate=256e9, record_length=1e-6, voltage_range=0.8
                ),
                "eye": ComplianceTestConfig(
                    test_pattern=self.test_patterns["eye"], sample_rate=256e9, record_length=2e-6, voltage_range=0.8
                ),
                "levels": ComplianceTestConfig(
                    test_pattern=self.test_patterns["levels"], sample_rate=224e9, record_length=500e-9, voltage_range=1.0
                ),
            }
